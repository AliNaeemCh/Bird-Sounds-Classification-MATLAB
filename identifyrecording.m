function identifyrecording
load('melfilterbanks');
load('birdpictures');
recorder=audiorecorder(getFs,16,1);
disp('Next 5 seconds will be recorded!');
record(recorder);
pause(5)
stop(recorder)
birdaudioin.inputaudio=getaudiodata(recorder);
...
disp(' ')
sound(birdaudioin.inputaudio,getFs);
end